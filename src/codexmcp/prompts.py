...
class PromptManager:
    ...
    def ensure_loaded(self):
        ...
        if not self._loaded:
            logger.debug("PromptManager: Prompts not loaded yet. Calling _load_prompts.")
            self._load_prompts()
        else:
            ...
            for name in importlib.resources.contents(resource_package):
                if name.endswith(".txt"):
                    ...
                    try:
                        ...
                        logger.error(f"Failed to load prompt {name} from {resource_package}: {str(e)}", exc_info=True)
            
            self._loaded = True # Set loaded to true AFTER attempting to load
            logger.info(f"PromptManager: Finished loading. {prompt_files_found} prompt templates loaded from {resource_package}.")
            if not self._prompts:
                logger.warning(f"PromptManager: No prompts were loaded from {resource_package}. Check directory and files.")

        except FileNotFoundError:
            logger.error(f"PromptManager: Prompt directory/package '{resource_package}' not found. Cannot load prompts.", exc_info=True)
            self._loaded = True # Still set to true to prevent reload attempts for a non-existent path
        except Exception as e:
            logger.error(f"PromptManager: Failed to load prompts directory '{resource_package}': {str(e)}", exc_info=True)
    ...
    def get(self, name: str, **kwargs) -> str:
        ...
        if name not in self._prompts:
            logger.error(f"Prompt template '{name}' not found after ensuring loaded. Available: {list(self._prompts.keys())}")
